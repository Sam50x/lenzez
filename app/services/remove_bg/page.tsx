...
import { addImageToSupabaseTable, removeBackgroundImage, getUser, urlToFile } from "@/app/lib/actions"
...
const RemoveBg = () => {
    ...
    const handleBgRemoving = async () => {
        ...
        try {
            const afterImageUrl = await removeBackgroundImage(inputInfo.image)
    ...
    return (
        <div className='flex flex-col mt-8 px-12'>
            <div className="flex flex-row justify-start items-start flex-wrap gap-6">
                ...
                <div className="w-fit h-fit" onClick={handleBgRemoving}>
                    <SolidBtn title={isRemoving ? 'Removing Bg...' : 'Remove Bg'} isDisabled={!inputInfo.image || isRemoving ? true : false} />
                </div>
            </div>
