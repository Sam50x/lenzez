...
import { addImageToSupabaseTable, removeObjectImage, getUser, urlToFile } from "@/app/lib/actions"
...
const RemoveObj = () => {
    ...
    const handleBgReplacing = async () => {
        ...
        try {
            const afterImageUrl = await removeObjectImage(inputInfo.image, inputInfo.prompt)
            ...
            <div className="flex flex-row justify-start items-start flex-wrap gap-6">
                <div className="flex flex-row justify-start items-start flex-wrap gap-2">
                    ...
                    <div className="w-fit h-fit">
                        <OutlineBtn>
                            <input type="text" onChange={handleInfoChange} placeholder="eg. Head" className="text-text" name="prompt" id="prompt" value={inputInfo.prompt} />
                        </OutlineBtn>
                    </div>
                </div>
                <div className="w-fit h-fit" onClick={handleBgReplacing}>
                    <SolidBtn title={isRemoving ? 'Removing Obj...' : 'Remove Obj'} isDisabled={!inputInfo.image || isRemoving ? true : false} />
                </div>
            </div>
