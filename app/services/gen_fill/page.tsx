...
import { addImageToSupabaseTable, genFillImage, getUser, urlToFile } from "@/app/lib/actions"
...
const GenFill = () => {
    ...
    const [isGenFilling, setIsGenFilling] = useState<boolean>(false)
    ...
    const handleGenFilling = async () => {
        if (!inputInfo.image) {
            console.error('No image was provided')
            return
        }

        setIsGenFilling(true)

        try {
            const afterImageUrl = await genFillImage(inputInfo.image, inputInfo.ratio)
    ...
    return (
        <div className='flex flex-col mt-8 px-12'>
            <div className="flex flex-row justify-start items-start flex-wrap gap-6">
                ...
                <div className="w-fit h-fit" onClick={handleGenFilling}>
                    <SolidBtn title={isGenFilling ? 'Gen Filling...' : 'Gen Fill'} isDisabled={!inputInfo.image || isGenFilling ? true : false} />
                </div>
            </div>
